...
func parsePrizeLine(line: string): Coord =
  ...
  result.x = matches[0].parseBiggestInt + correction
  result.y = matches[1].parseBiggestInt + correction
...
proc solve(s: Scenario): int =
  var A = ((s.prize.x * s.B.y) - (s.B.x * s.prize.y)) / ((s.B.y * s.A.x) - (s.A.y * s.B.x))
  if floor(A) != A:
    return 0
